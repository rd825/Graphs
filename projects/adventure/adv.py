...
# create a dict in which we'll map out the maze
maze_map = {}
...
for exit in player.currentRoom.getExits():
    uncharted.add(f"Room {player.currentRoom.id}: {exit}")

while uncharted:  # let's do DFS, as specified in lecture
    if '?' in maze_map[player.currentRoom.id].values():
        direction = None
        start_room = player.currentRoom.id

        # find an uncharted exit and make it the next direction to go in
        if 'n' in maze_map[start_room] and maze_map[start_room]['n'] == '?':
            direction = 'n'
        elif 'e' in maze_map[start_room] and maze_map[start_room]['e'] == '?':
            direction = 'e'
        elif 's' in maze_map[start_room] and maze_map[start_room]['s'] == '?':
            direction = 's'
        elif 'w' in maze_map[start_room] and maze_map[start_room]['w'] == '?':
            direction = 'w'

        # remove the next room from our uncharted rooms
        uncharted.remove(f"Room {player.currentRoom.id}: {direction}")
        # go to the next room
        player.travel(direction)

        # add new room to our traversal path
        new_room = player.currentRoom.id
        traversalPath.append(direction)

        # if the current room isn't in our map, grab the exits
        if new_room not in maze_map:
            maze_map[new_room] = {
                x: '?' for x in player.currentRoom.getExits()}

        # update the maze map
        maze_map[start_room][direction] = new_room
        maze_map[new_room][backtrack(direction)] = start_room

        # log unexplored rooms in uncharted
        for exit_direction, room_id in maze_map[new_room].items():
            if room_id == '?':
                uncharted.add(f"Room {new_room}: {exit_direction}")

        # remove the current room's backward pointer from uncharted
        if f"Room {new_room}: {backtrack(direction)}" in uncharted:
            uncharted.remove(f"Room {new_room}: {backtrack(direction)}")

    # if there are no unexplored exits, backtrack
    else:
        # set the start room to be the room we're stuck in
        start_room = player.currentRoom.id
        queue = Queue()

        for exit_direction, room_id in maze_map[start_room].items():
            # enqueue exit directions and the corresponding rooms
            queue.put([[exit_direction, room_id]])

        # this is where BFS happens
        while not queue.empty():
            # returns the backtrack path
            path = queue.get()
            # returns the next step in the path
            current = path[-1]

            # if there's an uncharted exit in the current room's exits
            if '?' in [room_id for exit_direction, room_id in maze_map[current[1]].items()]:
                # backtrack along the path and add it to the traversal path
                for exit_direction, room_id in path:
                    player.travel(exit_direction)
                    traversalPath.append(exit_direction)
                break
            else:
                # grab the exits and the rooms they point to for the current room
                for exit_direction, room_id in maze_map[current[1]].items():
                    # if the exits don't lead to the room we're stuck in and don't lead to a room already in our path:
                    if room_id != start_room and room_id not in [room_id for exit_direction, room_id in path]:
                        # add the exit direction and the room it points to to the path
                        queue.put(list(path) + [[exit_direction, room_id]])
